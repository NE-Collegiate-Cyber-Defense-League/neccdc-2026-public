...
resource "aws_instance" "branch_pfsense" {
  ami                  = data.aws_ami.branch_pfsense.image_id
  ...
  network_interface {
    network_interface_id = var.branch_pfSense_interfaces.public
    device_index         = 0
  }

  network_interface {
    network_interface_id = var.branch_pfSense_interfaces.private
    device_index         = 1
  }
